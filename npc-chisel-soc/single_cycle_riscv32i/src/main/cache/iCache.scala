...
class iCache(val block_size: Int, val sets: Int, val ways: Int, val replacementPolicy: String) extends Module{
    ...
    n_state := MuxLookup(c_state, s_IDLE)(Seq(//second phase
        ...
        s_i_1            ->  Mux(io_flush.flush_flg && (~(io.out.rvalid & out_rready)), s_Flush, Mux(io_flush.flush_flg && (io.out.rvalid & out_rready), s_IDLE, (Mux((io.out.rvalid & out_rready), Mux(((c.U === 1.U || ~issdram_raddr) || (c.U =/= 1.U && count === 0.U)), s_i_2, Mux((c.U =/= 1.U && count =/= 0.U && out_arlen === 0.U), s_i_0, s_i_1)), s_i_1))),
